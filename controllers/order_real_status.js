...
const { OrderRealStatus } = require('../models')
...
controller.create = async (req, res) => {
  try {
    await OrderRealStatus.create(req.body)
    // HTTP 201: Created
    res.status(201).end()
  }
  catch(error) {
...
controller.retrieve = async (req, res) => {
  try {
    const data = await OrderRealStatus.findAll()
...
controller.retrieveOne = async (req, res) => {
  try {
    const data = await OrderRealStatus.findByPk(req.params.id)
...
controller.update = async (req, res) => {
  try {
    const response = await OrderRealStatus.update(
      req.body,
...
controller.delete = async (req, res) => {
  try {
    const response = await OrderRealStatus.destroy(
      { where: { id: req.params.id } }
    )
